...
public class TilePiece extends JButton {
  ...
  /**
   * Determines if a given TilePiece is adjacent to this TilePiece on the board. Tiles are known as
   * adjacent if they are next to each other either horizontally or vertically.
   *
  ...
  public boolean isAdjacentTo(TilePiece p) {
    ...
    if (((p.getXPos() == xPos) && Math.abs(p.getYPos() - yPos) == 1)
        || ((p.getYPos() == yPos) && Math.abs(p.getXPos() - xPos) == 1)) return true;
  ...
  /**
   * Determines if this tile is in its correct (solved position). This method assumes that the
   * puzzle is generated in the completed position.
   */
